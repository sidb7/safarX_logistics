...
const OrderFormB2B: React.FC<OrderFormB2BProps> = ({ onBoxDataUpdate }) => {
  ...
  const [isPackageCatalogModalOpen, setIsPackageCatalogModalOpen] = useState<boolean>(false);
  ...
  const [filteredCatalogProducts, setFilteredCatalogProducts] = useState<ProductCatalogItem[]>([]);
  ...
  const packageSearchRefs = useRef<{[packageId: string]: HTMLDivElement | null}>({});
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      Object.keys(showPackageSearchResults).forEach((packageId) => {
        if (packageSearchRefs.current[packageId] && 
          ...
          return {
            ...
            packages: box.packages.map((pkg) => {
              if (pkg.id === packageId) {
                ...
                if (field === 'quantity' || field === 'unitPrice' || field === 'unitWeight') {
                  const qty = Number(field === 'quantity' ? value : pkg.quantity) || 0;
                  const unitPrice = Number(field === 'unitPrice' ? value : pkg.unitPrice) || 0;
                  const unitWeight = Number(field === 'unitWeight' ? value : pkg.unitWeight) || 0;
  ...
  const handlePackageNameSearch = (boxId: number, packageId: number, value: string) => {
  ...
  const selectPackageFromSearch = (boxId: number, packageId: number, product: ProductCatalogItem) => {
...
{getFilteredPackages(box).map((pkg) => (
  <div key={pkg.id} className="p-4 border-b border-gray-200 last:border-b-0">
      ...
      <div className="mt-4">
        ...
        <div className="flex items-center mb-4">
          ...
          <div className="flex-grow flex items-center space-x-3">
            ...
            <div className="w-[40%] relative" ref={el => packageSearchRefs.current[`${box.id}-${pkg.id}`] = el}>
              <FloatingLabelInput
                placeholder="Package Name"
                value={pkg.name.toString()}
                onChangeCallback={(value) => handlePackageNameSearch(box.id, pkg.id, value)}
                icon={<img src={searchIcon} alt="Search" />}
              />
              
              {/* Package Name Search Results Dropdown */}
              {showPackageSearchResults[`${box.id}-${pkg.id}`] && 
               filteredPackageResults[`${box.id}-${pkg.id}`]?.length > 0 && (
                <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                  {filteredPackageResults[`${box.id}-${pkg.id}`].map((product) => (
                    <div 
                      key={product._id} 
                      className="p-3 hover:bg-gray-100 cursor-pointer border-b border-gray-100 flex justify-between items-center"
                      onClick={() => selectPackageFromSearch(box.id, pkg.id, product)}
                    >
                      <div>
            ...
            <div className="w-[10%]">
              <FloatingLabelInput
                ...
                type="number"
                onChangeCallback={(value) => updatePackageField(box.id, pkg.id, "quantity", value)}
              />
            </div>
            ...
            <div className="w-[10%]">
              <FloatingLabelInput
                ...
                type="number"
                onChangeCallback={(value) => updatePackageField(box.id, pkg.id, "unitPrice", value)}
              />
            </div>
            ...
            <div className="w-[15%]">
              <FloatingLabelInput
                ...
                type="number"
                onChangeCallback={(value) => updatePackageField(box.id, pkg.id, "unitWeight", value)}
              />
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 ml-3">
            <button 
              className={`p-2 ${pkg.isSaved ? "text-blue-500 cursor-not-allowed" : "text-gray-500 cursor-pointer"}`}
              onClick={() => savePackage(box.id, pkg.id)}
              title={pkg.isSaved ? "Package saved" : "Save package"}
              disabled={pkg.isSaved}
            >
              <Bookmark isFilled={pkg.isSaved} className="w-5 h-5" />
            </button>
            <button
            ...
            <OneButton
              ...
              onClick={() => openPackageCatalogModal(box.id, pkg.id)}
              variant="secondary"
