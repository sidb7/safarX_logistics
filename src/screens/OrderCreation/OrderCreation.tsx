...
const handleProceedToNextStep = async () => {
  ...
  const hasDeliveryErrors = Object.values(deliveryErrors).some(error => error);
  ...
  if ((order.orderType === "B2C" && boxesData.length === 0) || 
  ...
  try {
    ...
    if (!response?.data?.success) {
      toast.error(response?.data?.message || "Failed to submit pickup information");
      return;
    }
    ...
    if (!deliveryResponse?.data?.success) {
      toast.error(deliveryResponse?.data?.message || "Failed to submit delivery information");
      return;
    }
    ...
    return boxes.map((box, index) => {
      ...
      if (order.orderType === "B2C") {
        ...
        return {
          ...
          codInfo: {
            isCod: paymentMethod === "Cash on Delivery",
            collectableAmount: paymentMethod === "Cash on Delivery" ? Number(collectibleAmount) || 0 : 0,
