...
import OneButton from "../../components/Button/OneButton";
import CenterModal from "../../components/CustomModal/customCenterModal";
import { GET_TRANSPORTER_ID, POST_SET_ORDER_ID } from "../../utils/ApiUrls";
import { POST, GET } from "../../utils/webService";
import FloatingLabelInput from "./FloatingLabelInput";
import truckIcon from "../../assets/Order/RTO.svg";
import checkIcon from "../../assets/Order/check.svg";
...
interface EWayBillCardProps {
  ...
  orderId?: string;
  orderType?: string;
  onEWayBillUpdate?: (billNumber: string) => void;
  isServiceSelected?: boolean; // New prop to check if a shipping service is selected
  partnerName?: string; // New prop for selected service name
  serviceId?: string; // Add service ID to track changes
}
...
const EWayBillCard: React.FC<EWayBillCardProps> = ({
  ...
  onEWayBillUpdate = () => {},
  isServiceSelected = false, // Default to false
  partnerName = "", // Default to empty string
  serviceId = "", // Service ID to track changes
}) => {
  // States
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [transporters, setTransporters] = useState<Transporter[]>([]);
  ...
  useEffect(() => {
    // Only reset if we have a valid partner name and it's different from the last one
    if (partnerName && partnerName !== lastPartnerName && lastPartnerName !== "") {
      // Partner has changed, reset E-Way Bill state to the initial state with button enabled
      setEWayBillAdded(false);
    ...
    // Always update the last partner name if there is a valid one
    if (partnerName) {
      setLastPartnerName(partnerName);
    }
  }, [partnerName, lastPartnerName, onEWayBillUpdate]);
  ...
  const fetchTransporters = async () => {
    ...
    try {
      const payload = {
        ...
        pageNo: 1,
        sort: { _id: -1 },
        orderType: orderType || "B2C", // Ensure orderType is B2C if not provided
        partnerName: partnerName || "" // Include the selected service name
      };
  ...
  const handleEnterEWayBillDetails = () => {
    // Only open the modal if a service is selected
      ...
      <>
        <div className="bg-[#FFFBEB] rounded-lg shadow-md p-6 mb-6">
          <div className="flex items-start">
            ...
            {/* Button - Modified to be disabled when no service is selected */}
            <div className="ml-4">
              <OneButton
                text={isServiceSelected ? "Enter E-Way Bill Details" : "Select Service to Proceed"}
                onClick={handleEnterEWayBillDetails}
                variant="primary"
                className="!rounded-full"
