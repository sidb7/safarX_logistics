...
function OrderBooked() {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      if (response?.data?.status && response?.data?.data?.length > 0) {
        // Store all order data objects
        const allOrderData = response.data.data[0].data || [];
        setOrderDetails(allOrderData);
        console.log("Order details found:", allOrderData);
      } else {
        console.error("API Error or no data:", response?.data);
      }
    } catch (error) {
    ...
    orderDetails.forEach((order) => {
      if (order?.status && order.status.length > 0) {
        ...
        if (orderLatestStatus?.timeStamp > latestTimestamp) {
          latestTimestamp = orderLatestStatus.timeStamp;
          latestStatus = orderLatestStatus.currentStatus || "Pending";
        }
      }
    });
  ...
  // Get all AWB numbers from all order objects
  const getAllAwbNumbers = (): string[] => {
    // First, check if AWB numbers were passed from OrderCreation
    if (passedData?.awbNumbers && passedData.awbNumbers.length > 0) {
      // return passedData.awbNumbers;
  ...
  // Get customer name, address, contact information from the first order
  const getCustomerName = (): string => {
    return orderDetails[0]?.deliveryAddress?.contact?.name || "N/A";
  };
  ...
  // Get courier name from the first order
  const getCourierName = (): string => {
    return orderDetails[0]?.service?.partnerName || "N/A";
  };
...
//   // Handle email
//   const handleEmail = () => {
//     const customerEmail = orderDetails[0]?.deliveryAddress?.contact?.emailId || '';
//     if (customerEmail) {
//       // You can customize this based on your backend email service
//       // This is just a placeholder for the email functionality
//       console.log('Email functionality to be implemented for:', customerEmail);
