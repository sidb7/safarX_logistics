...
//       badges.push({ text: 'Best Value', color: 'bg-blue-100 text-[#160783]' });
//     }
...
//                     className="h-5 w-5 text-[#160783] border-gray-300 focus:ring-blue-500"
...
const ShippingServiceSelector: React.FC<ShippingServiceSelectorProps> = ({
    ...
    if (selected && tempOrderId && orderSource) {
      ...
      try {
        ...
        if (response?.data?.success) {
          ...
          toast.error(
            response?.data?.message || "Failed to select shipping service"
          );
          // Reset selection if API call fails
          setSelectedService(null);
        }
  ...
  const getServiceBadges = (serviceId: string) => {
    ...
    if (serviceId === fastestServiceId) {
      badges.push({
        text: "Fastest",
        color: "bg-green-100 text-green-600 border border-green-600",
      });
    }
    ...
    if (serviceId === bestValueServiceId) {
      badges.push({
        text: "Best Value",
        color: "bg-blue-100 text-[#160783] border border-blue-600",
      });
    }
  ...
  const getServiceModeBadge = (mode: string) => {
    if (mode === "AIR") {
      return {
        text: "Air",
        color:
          "bg-[#F7F7F8] text-black font-semibold text-sm leading-5 text-center align-middle capitalize font-open px-4 border border-gray-300",
      };
    } else if (mode === "SURFACE") {
      return {
        text: "Surface",
        color:
          "bg-[#F7F7F8] text-black font-semibold text-sm leading-5 text-center align-middle capitalize font-open border border-gray-300",
      };
    }
    return null;
  };
