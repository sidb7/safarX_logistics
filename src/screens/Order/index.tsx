...
const Index = () => {
    ...
    const channelReduxData = useSelector(
        (state: any) => state?.channel?.channel
    );
    ...
    const handleSyncOrder = async () => {
        try {
            if (syncChannelText.includes("Sync Channel")) {
                ...
                syncRef.current.childNodes[1].textContent =
                    "Sync In Progress...";
            ...
            if (data?.success) {
                ...
                setTimeout(() => {
                    window.location.href =
                        "/orders/view-orders?activeTab=draft";
                    window.onload = () => {
        ...
        return (
            <div
                className={
                    className
                        ? className
                        : `flex items-center mx-5 mt-2 justify-between`
                }
            >
                <div>
                    <CustomButton
    ...
    const getSellerOrderByStatus = async (
        ...
        try {
            ...
            const { orderCount, draftCount, failedCount, errorCount } =
                data?.data[0];
            ...
            if (updatedCount === undefined) {
                    ...
                    statusListFromApi?.forEach((e1: any) => {
                        ...
                        for (let index = 0; index < tempArr.length; index++) {
                            ...
                            if (element1.value === e1._id?.toUpperCase()) {
                                element1.orderNumber =
                                    e1?.count?.toLocaleString("en-US", {
                                ...
                                if (dateFilter === true) {
                                    const num: any = 0;
                                    element1.orderNumber = num.toLocaleString(
                                        "en-US",
                                        {
                                            minimumIntegerDigits: 2,
                                            useGrouping: false,
                ...
                for (let index = 0; index < tempArr.length; index++) {
                    ...
                    if (value === currentStatus) {
                        element.orderNumber = updatedCount.toLocaleString(
                            "en-US",
                            {
                                minimumIntegerDigits: 2,
                                useGrouping: false,
    ...
    const handleTabChanges = async (
        ...
        try {
            ...
            console.error(
                "An error occurred in handleTabChanges function:",
                error
            );
        }
    };
                ...
                if (juspayOrderId) {
                    ...
                    if (orderStatus?.data?.success === false) {
                        ...
                        let paymentPayload: any =
                            getLocalStorage("paymentErrorObject");
                        if (paymentPayload) {
    ...
    return (
        <>
            ...
            <DeleteModal
                ...
                deleteTextMessage={warningMessageForCancel(
                    cancellationModal?.payload
                )}
                    ...
                    {isSyncModalLoading ? (
                        ...
                        channelReduxData.map((elem: any) => (
                            <div className="mt-[1rem] mx-[1rem] p-[1rem] items-center flex flex-col border-4 rounded-md">
                                ...
                                <div
                                    className={`relative progress-bar mb-[1rem] `}
                                >
                                    <div
