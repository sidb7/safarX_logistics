...
const LostDamagedFilter: React.FC<LostDamagedFilterProps> = ({
  ...
  useEffect(() => {
    ...
    if (filterState?.name === "Current Tag" && filterState?.menu?.length > 0) {
      setCurrentTagChecks({
        lostDamaged: filterState.menu.some(
          (item) => item.value === "LOST/DAMAGE" && item.isActive
        ),
        lost: filterState.menu.some(
          (item) => item.value === "LOST" && item.isActive
        ),
        damaged: filterState.menu.some(
          (item) => item.value === "DAMAGE" && item.isActive
        ),
      });
    }
    // Then check if we're receiving an Actions filter update
      ...
      setActionsChecks({
        claimRaised: filterState.menu.some(
          (item) => item.value === true && item.isActive
        ),
        claimPending: filterState.menu.some(
          (item) => item.value === false && item.isActive
        ),
      });
    }
  }, [filterState]);
  ...
  const getActiveFiltersCount = () => {
    return (
      Object.values(currentTagChecks).filter(Boolean).length +
    ...
    <div className="w-full bg-white rounded-lg h-full flex flex-col">
      <div className="flex-1 overflow-y-auto">
        ...
        <div
          className={`mb-3 border rounded-lg ${
            isCurrentTagOpen ? "border-[#E8E8E8]" : "border-gray-200"
          }`}
        >
          <button
            onClick={() => setIsCurrentTagOpen(!isCurrentTagOpen)}
            className={`w-full flex items-center justify-between p-3 ${
              isCurrentTagOpen ? "bg-[#F6F6F6]" : "bg-white"
            } rounded-lg`}
          >
            <span className="font-Open font-semibold text-[16px] leading-[22px]">
              Current Tag
            </span>
            <img
              src={AccordianUp}
        ...
        <div
          className={`mb-3 border rounded-lg ${
            isActionsOpen ? "border-[#E8E8E8]" : "border-gray-200"
          }`}
        >
          <button
            onClick={() => setIsActionsOpen(!isActionsOpen)}
            className={`w-full flex items-center justify-between p-3 ${
              isActionsOpen ? "bg-[#F6F6F6]" : "bg-white"
            } rounded-lg`}
          >
            <span className="font-Open font-semibold text-[16px] leading-[22px]">
              Actions
      ...
      <div className="mt-4 mb-4 flex justify-center">
        {getActiveFiltersCount() > 0 && (
          <div className="bg-blue-50 text-[#160783] rounded-full px-3 py-1 text-sm">
