...
const Buttons = (className?: string, updateUserHandler?: any, disabled?: boolean) => {
...
function UpdateUser() {
    ...
    const onChangeAddUserCardHandler = (
        fieldName: any,
        value: string
    ) => {
    ...
    const copyDataFromHandler = (e: any) => {
        roleData?.forEach((role: any) => {
            if (e.target.value === role.roleId) {
                setAddUserData((prevData: any) => ({
                    ...prevData, roleId: role.roleId, roleName: role.roleName
                }))
            }
        });
    };
    ...
    const updateUserHandler = async () => {
        try {
            const { data: response }: any = await POST(POST_UPDATE_USER_DATA, addUserData);
            if (response?.success) {
    ...
    return (
        <div>
            <Breadcum label='UPDATE USER' component={Buttons("", updateUserHandler)} />
