export function newlyOnboard() {
  cy.visit("https://sysellerdev.yaarilabs.com");
  cy.wait(2000);
  cy.xpath("//*[text()='Sign Up']").click();
  cy.xpath("//*[text()='Sign up to create your account']").should("be.visible");
...
export function MobileOtpCreation() {
  ...
  cy.request({
    ...
    cy.xpath("//*[text()='Enter Mobile OTP ']//preceding-sibling::input").type(
      otpValue
    );
...
export function Questionnaire() {
  ...
  cy.xpath("//input[@title='Checkbox']").then(($checkboxes) => {
    ...
    cy.xpath("//input[@title='Checkbox']").then(($checkboxes) => {
      ...
      cy.get("#selectDropdown")
        .should("be.visible")
        .then(($dropdown) => {
          const options = $dropdown.children("option");
          const randomIndex = Math.floor(Math.random() * options.length);
          const randomOption = options.eq(randomIndex).text().trim();
          cy.log(`Selected random option: ${randomOption}`);
...
export function KYC() {
  ...
  cy.xpath("(//*[@value='individual']/../parent::div//parent::div)[1]").click();
  cy.wait(2000);
